...
langs.reverse()                     # 리스트 순서를 거꾸로 뒤집기(원본 변경)
print(langs)

langs.sort()                        # 오름차순 정렬 (원본 변경)
print(langs)
...
# 얕은 복사(shallow copy) => 주소값 복사 vs 깊은 복사(deep copy)
ori = [[1, 2], [3, 4]]

result =ori.copy()
result[0].append(10)        #원본 바뀜
...
result = copy.deepcopy(ori) # 사본 바뀜(원본 유지)
result[0].append(100)
...
# result = [sum(score) / len(score) for score in zip(st1,st2,st3)]
